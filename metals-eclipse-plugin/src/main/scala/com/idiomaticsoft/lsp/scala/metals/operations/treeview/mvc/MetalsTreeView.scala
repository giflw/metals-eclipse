...
class MetalsTreeView extends ViewPart {
  ...
  private var viewsTo
      : scala.collection.mutable.HashMap[String, (ExpandItem, TreeViewer)] =
  ...
  override def createPartControl(parent: Composite) = {
    ...
    val resourceStream = getClass().getClassLoader
      .getResourceAsStream("icons/full/etools/crosshair.png")
        ...
        if (Option(editorPart).isDefined) {
          ...
          if (target.isInstanceOf[ITextViewer]) {
            ...
            val textDocumentPositionParams =
              LSPEclipseUtils.toTextDocumentPosistionParams(offset, document)
        ...
        Option(evt.item.getData()).map({ data =>
          ...
          controller
            ...
            .treeViewVisibilityDidChange(
              new TreeViewVisibilityDidChangeParams(
                data.asInstanceOf[TreeViewNode].getViewId(),
                false
              )
            )
        })
  ...
  def createViewer(parent: Composite, viewId: String) = {
        ...
        obj match {
          case tvn: TreeViewNode =>
            Option(tvn.getIcon())
              .map({
                icon =>
                  val someResourceStream = Option(
                    getClass().getClassLoader
                      .getResourceAsStream(s"icons/metals/$icon.png")
                  )
                  someResourceStream
                    .map({ resourceStream =>
    ...
    viewer.addDoubleClickListener(new IDoubleClickListener {
      def doubleClick(evt: DoubleClickEvent): Unit = {
        for {
          selection <- Option(
            evt.getSelection().asInstanceOf[IStructuredSelection]
          )
          obj <- if (!selection.isEmpty()) Option(selection.getFirstElement())
          else None
          ...
          lsp4jcommand.setArguments(
            java.util.Arrays.asList(command.getArguments())
          )
          controller
            .languageServer()
            .getWorkspaceService()
            .executeCommand(lsp4jcommand)
        }
      }
    })
    viewer
